...
//		int seqno = service.getSeqnoWithNextval();  // 오라클 버전
...
//					file.delete();
					...
					//파일 테이블에서 파일 정보 삭제
					// 게시물 수정에서 삭제할 파일 목록이 전송되면 이 값을 받아서 tbl_file 내에 있는 파일정보를 하나씩 삭제하는 deleteFileList 실행
		...
		if(!multipartFile.isEmpty()) {  // 파일 등록 및 수정 시 파일 업로드
			...
			for(MultipartFile mpr:multipartFile) {  // 파일 여러개의 리스트 값이 들어오니까 for문 돌려
...
//		System.out.println("ViewUserid = " + view.getUserid());
...
//		model.addAttribute("view", mapper.view(seqno));
		model.addAttribute("view", view);  // 글번호
		model.addAttribute("page", pageNum);  // 페이지번호
		model.addAttribute("keyword", keyword);  // 검색 
		model.addAttribute("pre_seqno", service.pre_seqno(seqno, keyword));  // 이전글 
		model.addAttribute("next_seqno", service.next_seqno(seqno, keyword));  // 다음글 
		model.addAttribute("likeCheckView", likeCheckView);  // 좋아요, 싫어요 확인 내용
		model.addAttribute("fileListView", service.fileListView(seqno));  // 파일 상세보기
	}
...
//		BoardVO board = mapper.view(seqno);
	...
	public Map<String, Object> postLikeCheck(@RequestBody Map<String, Object> likeCheckData) throws Exception {
		...
		String likeDate = "";
		String dislikeDate = "";
		if(likeCheckData.get("mylikecheck").equals("Y"))  // Y이면
			likeDate = LocalDateTime.now().toString();  // 현 시간을 string으로 바꿔서 넣어준다.
...
//		return "{\"likeCnt\":\"" + likeCnt + "\",\"dislikeCnt\":\"" + dislikeCnt + "\"}";
		// 위에처럼 return하면 위에 public Map<String, Object> 이거를 public String으로 바꿔줘야함.
	}
	...
	public List<ReplyVO> postReply(@RequestBody ReplyVO reply,@RequestParam("option") String option)throws Exception{
		...
		return service.replyView(reply);  // 값을 받고 json 형태로 다시 보낸다.(view.jsp)
	}
